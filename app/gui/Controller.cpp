        ...
        case ViewPanel::T_Viewport:
            {
                ...
                QObject::connect(nodeModel, SIGNAL(modified()), viewportWidget, SLOT(update()));
                QObject::connect(m_sceneModel, SIGNAL(triggerRedraw()), viewportWidget, SLOT(update()));
                QObject::connect(m_sceneModel, SIGNAL(selectionChanged(bool)), viewportWidget, SLOT(update()));
                ...
                //QObject::connect(m_sceneModel, SIGNAL(cameraCreated(const QString &)), viewportPanel, SLOT(addCamera(const QString &)));
                //QObject::connect(m_sceneModel, SIGNAL(cameraDeleted(const QString &)), viewportPanel, SLOT(removeCamera(const QString &)));
                ...
                // update the camera selection drop down when scene changed
                //QObject::connect(m_sceneModel, SIGNAL(objectCreated(const QString&)), viewportPanel, SLOT(updateCameraSelection()));
                //QObject::connect(nodeModel, SIGNAL(objectDeleted(const QString&)), viewportPanel, SLOT(updateCameraSelection()));
        ...
        case ViewPanel::T_NetworkEditor:
            {
                ...
                // redraw the graphics scene if the node model has been updated
                QObject::connect(nodeModel, SIGNAL(modified()), networkGraphicsView, SLOT(update()));
        ...
        case ViewPanel::T_ParameterEditor:
            {
                ...
                QObject::connect(nodeModel, SIGNAL(nodeSelected(Node *)), parameterPanel, SLOT(showParameters(Node *)));
                ...
                // display the parameters of the first selected node in the new parameter panel
                QStringList selectedNodes = m_sceneModel->getSelectedObjects();
                if (selectedNodes.size() > 0) {
                    Node *selectedNode = nodeModel->getNode(selectedNodes.at(0));
                    parameterPanel->showParameters(selectedNode);
                }
            }
            break;
