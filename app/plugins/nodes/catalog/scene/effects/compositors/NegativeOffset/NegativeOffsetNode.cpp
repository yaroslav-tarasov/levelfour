...
//!
//! \file "NegativeOffsetNode.cpp"
//! \brief Implementation file for NegativeOffsetNode class.
//!

#include "NegativeOffsetNode.h"
...
//!
NegativeOffsetNode::NegativeOffsetNode ( const QString &name, ParameterGroup *parameterRoot ) :
    CompositorNode(name, parameterRoot)
{
    // create compositor
    if (m_viewport)
        m_compositor = Ogre::CompositorManager::getSingleton().addCompositor(m_viewport, "NegativeOffsetCompositor", 0);
...
//!
NegativeOffsetNode::~NegativeOffsetNode ()
{
}
...
//!
void NegativeOffsetNode::notifyMaterialRender(Ogre::uint32 pass_id, Ogre::MaterialPtr &mat)
{
...
//!
void NegativeOffsetNode::processOutputImage ()
{
    // obtain input image
    Ogre::TexturePtr inputTexture = getTextureValue("Input Map");
    if (inputTexture.isNull()) {
        ...
        Log::warning("No input image connected.", "NegativeOffsetNode::processOutputImage");
    }
