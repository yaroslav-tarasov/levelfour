...
//!
//! \file "NegativeNode.cpp"
//! \brief Implementation file for NegativeNode class.
//!
...
//!
NegativeNode::NegativeNode ( const QString &name, ParameterGroup *parameterRoot ) :
    CompositorNode(name, parameterRoot)
{
    // create compositor
    if (m_viewport)
        m_compositor = Ogre::CompositorManager::getSingleton().addCompositor(m_viewport, "NegativeCompositor", 0);
...
//!
void NegativeNode::notifyMaterialRender(Ogre::uint32 pass_id, Ogre::MaterialPtr &mat)
{
...
//!
void NegativeNode::processOutputImage ()
{
    // obtain input image
    Ogre::TexturePtr inputTexture = getTextureValue("Input Map");
    if (inputTexture.isNull()) {
        ...
        Log::warning("No input image connected.", "NegativeNode::processOutputImage");
    }
