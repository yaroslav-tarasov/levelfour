...
void ViewingParameters::reset ()
{
	...
	m_viewRadius = 65.0;
    m_viewAnglePhi = 90.0;  // elevation (90° means the camera is on ground level)
    m_viewAngleTheta = 90.0;  // rotation around Y axis
...
void ViewingParameters::applyTo ( Node *node )
{
    ...
    node->setValue("Position", position, true);
    node->setValue("Orientation", orientation, true);
    
    // TODO: NILZ: Maybe we need a triggerRedraw() here.

    // NILZ: Triggers rebuild of scene each time viewport is changed (when linked with cam)
    //node->setValue("Render Resolution > Render Width", m_canvasWidth, true);
    //node->setValue("Render Resolution > Render Height", m_canvasHeight, true);
    
    node->setValue("Orbit > Target Point", m_center, true);
...
void ViewingParameters::applyTo ( Ogre::Camera *camera )
{
    ...
    CameraInfo *cameraInfo = Ogre::any_cast<CameraInfo *>(((Ogre::MovableObject *) camera)->getUserAny());
    if (cameraInfo) {
        cameraInfo->width = m_canvasWidth;
        cameraInfo->height = m_canvasHeight;
        camera->setAspectRatio(float(cameraInfo->width) / cameraInfo->height);
    }
...
void ViewingParameters::applyFrom ( Node *node )
{
    ...
    m_center = node->getVectorValue("Orbit > Target Point");
    m_viewRadius = node->getDoubleValue("Orbit > Radius");
    m_viewAnglePhi = Ogre::Degree(node->getDoubleValue("Orbit > Azimuth angle"));
    m_viewAngleTheta = Ogre::Degree(node->getDoubleValue("Orbit > Zenith angle"));
