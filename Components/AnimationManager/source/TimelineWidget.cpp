...
//!
const QSize TimelineWidget::DefaultFrameSize = QSize(9, 33);
...
TimelineWidget::TimelineWidget ( QWidget *parent /* = 0 */, Qt::WindowFlags flags /* = 0 */ ) :
	...
	m_currentGrabber(new GrabberWidget(DefaultFrameSize, GrabberWidget::P_OnFrame, QColor(241, 112, 0), this)),
    m_movedGrabber(0),
    ...
    if (inEditingMode) {
        if (key == Qt::Key_Return || key == Qt::Key_Enter) {
            ...
            if (ok) {
                ...
                return; // Log::error("The entered index is not a number.", "TimelineWidget::on_m_currentFrameEdit_returnPressed");
...
void TimelineWidget::paintImage ( const QRect &rectangle )
{
    ...
    QColor midColor (125,125,125);
	QColor baseColor (61,63,66);
