...
ViewportPanel::ViewportPanel(QWidget * parent) : QTabWidget(parent), _scene(0)
{
	sceneLayout  = new QVBoxLayout;
	sceneStack = new QStackedWidget;
	sceneToolBar = new QToolBar;
	sceneSelection = new QComboBox;
	sceneCameras = new QComboBox;
	sceneDisplays = new QComboBox;
	sceneViewports = new QComboBox;
	osgOutputWidget = new QWidget;
	...
	connect(sceneSelection, SIGNAL(activated(int)),
...
//	connect(showaxisAction, SIGNAL(triggered()), this, SLOT(toggleAxis()));
