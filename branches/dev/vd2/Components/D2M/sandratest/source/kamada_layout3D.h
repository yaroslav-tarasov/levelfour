#ifndef KAMADA_LAYOUT_H
#define KAMADA_LAYOUT_H
...
namespace graph {
	...
	*/
							...
							edge_or_side<false, T> length)
	{
		...
		for (vertex_iterator ui = vertices(g).first, end = vertices(g).second; ui != end; ++ui)
		{
	...
	struct linear_solver<3> {
		template <typename Vec>
		static Vec solve(double mat[3][3], Vec rhs) //In boost's version this is: static Vec solve(double mat[2][2], Vec rhs)
		{
	...
	struct kamada_kawai_spring_layout_impl
	{
		...
		//Constructor of the struct, just declares some parameters
		...
		deriv_type compute_partial_derivative(vertex_descriptor m, vertex_descriptor i)
		{
			...
			return result;
		} //end function compute_partial_derivative
		...
		deriv_type compute_partial_derivatives(vertex_descriptor m)
		{
			...
			BGL_FORALL_VERTICES_T(i, g, Graph) {
				...
				result += deriv;
			} //end BGL_FORALL_VERTICES_T
			return result;
		} //end function compute_partial_derivatives
		...
		bool run()
		{
			...
			weight_type edge_length = detail::graph::compute_edge_length(g, 
																		distance, 
																		index,
																		edge_or_side_length);
		...
		SpringStrengthMatrix spring_strength;
		PartialDerivativeMap partial_derivatives;
	}; //end struct kamada_kawai_spring_layout_impl
	...
	{
		//Constructor of the struct, just declares a few parameters
		layout_tolerance(const T& tolerance = T(0.001)) : tolerance(tolerance), 
...
private:
   ...
   */
	...
	bool kamada_kawai_spring_layout(
								...
								WeightMap weight, 
								const Topology& topology,
								detail::graph::edge_or_side<EdgeOrSideLength,
								T> edge_or_side_length,
								Done done,
								...
								PartialDerivativeMap partial_derivatives)
	{
		...
		detail::graph::kamada_kawai_spring_layout_impl< Topology,
														...
														PositionMap,
														WeightMap,
														detail::graph::edge_or_side<EdgeOrSideLength,T>,
														Done,
														VertexIndexMap,
														DistanceMatrix,
		...
		return alg.run();
	} //end this version of function kamada_kawai_spring_layout

  /**
   * \overload
   */
	template<typename Topology, typename Graph, typename PositionMap, typename WeightMap,
			typename T, bool EdgeOrSideLength, typename Done, typename VertexIndexMap>
	bool kamada_kawai_spring_layout(
									const Graph& g, 
									PositionMap position,
									WeightMap weight, 
									...
									VertexIndexMap index)
	{
		...
		return kamada_kawai_spring_layout(g,
										position,
										weight,
										topology,
										edge_or_side_length,
