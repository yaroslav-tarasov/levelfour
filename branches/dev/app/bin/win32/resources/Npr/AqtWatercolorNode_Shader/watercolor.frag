...
void main (void)
{
	...
	gl_FragColor.a = opacity * density.a;
	
	float darkening = computeGradient(inputMap, texCoord.xy + noiseVec * pixelSize.xy);	
	gl_FragColor.a += edgeDarkening * darkening * density.a;
		
	gl_FragColor.a += granulationIntensity * granulation * density.a;	
	
	gl_FragColor.a += paperIntensity * paper * density.a;

	
	vec3 color = density.rgb;
	
	//color is extra saturated at areas with high density... makes the color feeling more convincing
	vec3 colorSaturated = color + saturation * gl_FragColor.a * (color - vec3( (color.r + color.g + color.b) / 3.0));	
	
	gl_FragColor.rgb = colorSaturated;
