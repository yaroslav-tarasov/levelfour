...
//!
//! Macro definition for fmx/09-specific additions.
//!
//#define FMX_09

//!
//! Macro definition for Borealis-specific additions.
//!
//#define BOREALIS
    ...
    m_documentationAction(new QAction(QIcon(":/helpIcon"), tr("&Documentation"), this))
{
    ...
    splashScreen.show();
    Qt::Alignment splashScreenAlignment = Qt::AlignHCenter | Qt::AlignTop;
    ...
    QFileInfoList fileInfoList = nodeDir.entryInfoList(QStringList() << "*.xml", QDir::Files);
    ...
    fileInfoList = widgetDir.entryInfoList(QStringList() << "*.xml", QDir::Files);
    if (fileInfoList.size() > 0)
        for (int i = 0; i < fileInfoList.size(); ++i) {
            QFileInfo fileInfo = fileInfoList.at(i);
        ...
        Log::warning(QString("No plugin files for widget types found in \"%1\".").arg(panelDir.path()), "Application::Application");
...
{
	Window *activeWindow = qobject_cast<Window *>(this->activeWindow());
    QString filename = QFileDialog::getSaveFileName(activeWindow, tr("Save Layout File"), QDir(m_currentFilename).path(), tr("Layout files (*.lt)"));
    if (filename.isEmpty())
        return false;
