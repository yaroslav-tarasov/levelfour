...
//!
//! \file "SepiaNode.cpp"
//! \brief Implementation file for SepiaNode class.
//!
...
//!
SepiaNode::SepiaNode ( const QString &name, ParameterGroup *parameterRoot ) :
    CompositorNode(name, parameterRoot)
{
    // create compositor
    if (m_viewport)
        m_compositor = Ogre::CompositorManager::getSingleton().addCompositor(m_viewport, "SepiaCompositor", 0);
...
//!
void SepiaNode::notifyMaterialRender(Ogre::uint32 pass_id, Ogre::MaterialPtr &mat)
{
...
//!
void SepiaNode::processOutputImage ()
{
    // obtain input image
    Ogre::TexturePtr inputTexture = getTextureValue("Input Map");
    if (inputTexture.isNull()) {
        ...
        Log::warning("No input image connected.", "SepiaNode::processOutputImage");
    }
