...
void ImageFileNode::resizeImageChanged()
{
	...
	if (scaleParameter) m_scaleValue = getIntValue("Scale in %");		
	// clear cache
	clearImageCache();	
}
...
void ImageFileNode::filenameChanged ()
{
    ...
    if (!prefix.isEmpty() || !suffix.isEmpty()) {
        ...
        QDir directory (path);
        QStringList files = directory.entryList(QStringList() << QString("%1*%2.png").arg(prefix).arg(suffix), QDir::Files, QDir::Name);
...
void ImageFileNode::processOutputImage ()
{
    ...
    if (!filename.endsWith(".png")) {
        Log::error("The image file has to be a PNG file.", "ImageFileNode::processOutputImage");
        return;
    }
    ...
    Ogre::Image *image;	
	bool useImageCache = getBoolValue("Use Image Cache");
    if (useImageCache && isCached(index)) {
        // load the image from cache
        image = getCachedImage(index);		
    } else {
        // load the image from file
        image = new Ogre::Image();
		image->load(filename.toStdString(), Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);	
		// Resize Image			
		if (m_scaleValue!=100) image->resize( 0.01*m_scaleValue*m_defaultSize.width(), 0.01*m_scaleValue*m_defaultSize.height() );					
		// write file to cache
		if (useImageCache)
	...
	// resize the default image to last loadet image size
    ...
    // destroy the image if it is not cached
    if (!useImageCache)
        delete image;
}
...
void ImageFileNode::decodeFilename ( const QString &filename, int *index, QString *prefix /* = 0 */, QString *indexString /* = 0 */, QString *suffix /* = 0 */ )
{
    ...
    if (indexExpression.indexIn(filename) != -1) {
        ...
        if (indexExpression.numCaptures() > 1)
