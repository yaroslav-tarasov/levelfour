...
{    
    ...
    */
    ...
    setObjectName(m_node->getName());
    setToolTip(m_node->getName());
...
void NodeGraphicsItem::paint ( QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget /* = 0 */ )
{
    ...
    //QString iconName = m_node->getNodeType().toLower();
    //if (m_node->getNodeType() == "Animation")
    //    iconName = "file";
    ...
    if (m_node)
        painter->drawText(QPointF(rect().left() + 4, rect().top() + 16), m_node->getName());
...
void NodeGraphicsItem::refresh ()
{
    if (m_node) {
        ...
        m_evalFlagItem->setChecked(m_node->isEvaluated());
        if (m_viewFlagItem)
            m_viewFlagItem->setChecked(dynamic_cast<ViewNode *>(m_node)->isViewed());
    }
...
void NodeGraphicsItem::contextMenuEvent ( QGraphicsSceneContextMenuEvent *event )
{
    ...
    evalFlagAction->setChecked(m_node->isEvaluated());
    connect(evalFlagAction, SIGNAL(toggled(bool)), m_node, SLOT(setEvaluate(bool)));

    // prepare view flag action
    QAction *viewFlagAction = 0;
    if (m_viewFlagItem) {
        ViewNode *viewNode = dynamic_cast<ViewNode *>(m_node);
...
{
    // TODO: NILZ: Somehow ItemPositionChange is always 0 with Qt 4.6.
...
void NodeGraphicsItem::updatePins ()
{
    ...
    ParameterGroup *parameterRoot = m_node->getParameterRoot();
        ...
        case PDM_All:
            {
                ...
                m_node->fillParameterLists(&inputParameters, &outputParameters);

                // iterate over the list of input parameters
                for (int i = 0; i < inputParameters.size(); ++i)
                    if (inputParameters[i]->isGroup())
                        m_inputPins.append(new PinGraphicsItem(this, i, dynamic_cast<ParameterGroup *>(inputParameters[i]), Parameter::PT_Input));
                    else
                        m_inputPins.append(new PinGraphicsItem(this, i, dynamic_cast<Parameter *>(inputParameters[i])));

                // iterate over the list of output parameters
                for (int i = 0; i < outputParameters.size(); ++i)
					if (outputParameters[i]->isGroup()) {
						PinGraphicsItem* newItem = new PinGraphicsItem(this, i, dynamic_cast<ParameterGroup *>(outputParameters[i]), Parameter::PT_Output);
						//newItem->setObjectName(outputParameters[i]->getName()); /// seim was here !!!
                        m_outputPins.append(newItem);
					}
					else {
						PinGraphicsItem* newItem = new PinGraphicsItem(this, i, dynamic_cast<Parameter *>(outputParameters[i]));
        ...
        case PDM_Connected:
            {
                ...
                m_node->fillParameterLists(&connectedInputParameters, &connectedOutputParameters, &unconnectedInputParametersAvailable, &unconnectedOutputParametersAvailable);
                ...
                for (int i = 0; i < connectedInputParameters.size(); ++i)
                    if (connectedInputParameters[i]->isGroup())
                        m_inputPins.append(new PinGraphicsItem(this, i, dynamic_cast<ParameterGroup *>(connectedInputParameters[i]), Parameter::PT_Input, true));
                    else
                        m_inputPins.append(new PinGraphicsItem(this, i, dynamic_cast<Parameter *>(connectedInputParameters[i])));
                ...
                for (int i = 0; i < connectedOutputParameters.size(); ++i)
                    if (connectedOutputParameters[i]->isGroup())
                        m_outputPins.append(new PinGraphicsItem(this, i, dynamic_cast<ParameterGroup *>(connectedOutputParameters[i]), Parameter::PT_Output, true));
                    else
                        m_outputPins.append(new PinGraphicsItem(this, i, dynamic_cast<Parameter *>(connectedOutputParameters[i])));
...
void NodeGraphicsItem::endCreatingConnection ( PinGraphicsItem *pinItem, Parameter *parameter )
{
    if (pinItem && parameter) {
        ...
        if (firstNode) {
            emit connectionRequested(firstNode, m_node);
        }

        ConnectionGraphicsItem::setTempItem(0);

        //NILZ: TODO: find a better and cleaner way to destroy connection
        if (tempConnectionItem) {
            //removeConnectionItem(tempConnectionItem);
            //otherNodeGraphicsItem->removeConnectionItem(tempConnectionItem);
            delete tempConnectionItem;
            //scene()->removeItem(tempConnectionItem);
